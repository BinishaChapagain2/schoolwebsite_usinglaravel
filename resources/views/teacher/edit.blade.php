...
<div class="w-full px-4 py-4 bg-yellow-600 shadow-md logo text-white flex justify-between">
    <div class="left items-center flex space-x-2">
      ...
      <span class="font-medium logo-name text-sm sm:text-base md:text-lg lg:text-xl">Edit Teacher</span>
  </div>
@endsection
...
<div class="mx-auto">
    ...
    <form action="{{ route('teacher.update',$teacher->id) }}" class="mt-8" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="grid w-full flex-1 grid-cols-1 gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div>
                ...
                <select name="gender" id="gender"
                    ...
                    <option value="Female" @if ($teacher->gender=='Female')
                        Selected
                    @endif>Female</option>
                    <option value="Other" @if ($teacher->gender=='Other')
                        Selected
                    @endif>Other</option>
                </select>
                @error('gender')
            ...
            <!-- Date of Joining -->
            <div>
                <label for="date_of_joining" class="block mb-2 text-sm font-semibold text-blue-700">Date of Joining</label>
            ...
            <!-- Marital Status -->
            <div>
                <label for="marital_status" class="block mb-2 text-sm font-semibold text-blue-700">Marital Status</label>
                <select name="marital_status" id="marital_status"
                    class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">
                    <option value="">Select Status</option>
                    <option value="Married" @if ($teacher->marital_status=='Married')
                        Selected
                    @endif>Married</option>
                    <option value="Unmarried" @if ($teacher->marital_status=='Unmarried')
                        Selected
            ...
            <div>
                ...
                <p>Current Picture:</p>
                <img src="{{ asset('images/teacher/' . $teacher->photopath) }}" alt="teacher photo" class="w-44">
            </div>
            ...
            <!-- Qualification -->
            <div>
                <label for="qualification" class="block mb-2 text-sm font-semibold text-blue-700">Qualification</label>
                <textarea name="qualification" id="qualification" placeholder="Qualification" rows="4"
                    class="w-full px-4 py-2 text-gray-700 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">{{$teacher->qualification}} </textarea>
                @error('qualification')
                    <p class="text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <!-- Work Experience -->
            <div>
                <label for="work_experience" class="block mb-2 text-sm font-semibold text-blue-700">Work Experience</label>
                <textarea name="work_experience" id="work_experience" placeholder="Work Experience" rows="4"
                    class="w-full px-4 py-2 text-gray-700 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">{{ $teacher->work_experience  }}</textarea>
            ...
            <div>
                ...
                <input type="password" name="password" id="password" placeholder="Password"  value="{{ $teacher->password  }}"
                    class="w-full px-4 py-2 text-gray-700 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">
                @error('password')
                    <p class="text-sm text-red-500">{{ $message }}</p>
                @enderror
        ...
        <div class="flex justify-center gap-4 mt-10">
            <input type="submit" value="Update Teacher" class="px-6 py-3 font-semibold text-white bg-blue-900 rounded-lg hover:bg-blue-700 cursor-pointer">
            <a href="{{ route('teacher.index') }}" class="px-6 py-3 font-semibold text-white bg-red-700 rounded-lg hover:bg-red-500">Exit</a>
        </div>
    </form>
</div>
@endsection
